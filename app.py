...
import llm_model  # Assuming llm_model is a module you've created for handling the model logic
...
def index_document(_llm_object, file_path):
    if file_path is not None:
        st.success(f"File '{file_path}' is being used!")
...
def init_state():
    ...
    if "repetition_penalty" not in st.session_state:
        st.session_state.repetition_penalty = 1
...
def sidebar():
    with st.sidebar:
        ...
        # Initialize the LLM object from the model
        st.session_state.llm_object = load_model()
        st.markdown("---")

        # Use the specified file path
        predefined_file_path = r"Addmition data MUL.pdf"
        _, retriever = index_document(st.session_state.llm_object, predefined_file_path)
